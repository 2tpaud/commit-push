...
- **`/api/payment/create`**: 나이스페이 주문 생성 후 결제창 호출용 `orderId`, `amount`, `goodsName`, `buyerName`(users.full_name), `buyerEmail`(auth user.email) 반환. **상품명**은 기본명(예: CommitPush Pro 1년)에 **이용 기간**(시작일~종료일)을 붙여 전달(결제일/만료일 기준 로직은 return과 동일). 클라이언트는 이 값을 `requestPay`에 넘겨 PG에 전달.
- **`/api/payment/return`**: 결제 완료/취소 후 콜백. GET은 취소로 간주. POST는 `authResultCode='0000'`이고 메시지에 취소/실패 문구가 없을 때만 승인 API 호출 후 `payments`·`users.plan`·`plan_expires_at`·`notifications` 반영. **만료일**: 첫 구독·만료 후 = 결제일 기준 +1개월/12개월; 동일 플랜·만료일 미래 = 기존 만료일 기준 +1개월/12개월(return·webhook 동일). **승인 방식**: 나이스페이 **REST 승인 API** `POST {NICE_PAY_API_BASE}/v1/payments/{tid}`를 사용하며, 인증은 **Basic** `Authorization: Basic base64(clientId:secretKey)`만 사용한다(Access Token/Bearer/OAuth 흐름 없음). 응답에서 `resultCode='0000'`, `amount` 일치, `status='paid'` 또는 `paidAt` 존재를 모두 만족해야 승인으로 간주. PG 리다이렉트 시 세션 쿠키가 없으므로 **세션 없이** `SUPABASE_SERVICE_ROLE_KEY`로 `order_id` 기준 결제 조회·갱신. 응답은 PC/모바일 동일하게 **200 + HTML**(클라이언트 리다이렉트로 `/plan` 이동).
- **`/api/payment/cancel`**: 결제 승인 후 취소(환불) 처리. 인증: 쿠키 세션 우선, 없으면 `Authorization: Bearer`. 요청 사용자 본인 결제 건(`payments.id`)만 처리하며, `status='paid'`이고 `paid_at` 기준 24시간 이내일 때만 나이스페이 **REST 취소 API** `POST {NICE_PAY_API_BASE}/v1/payments/{tid}/cancel`을 호출한다. 바디는 `{ orderId: payment.order_id, amount: String(payment.amount), reason }` JSON, 인증은 승인과 동일하게 Basic 헤더만 사용한다. 응답의 `resultCode='0000'`일 때 `payments.status='cancelled'`로 갱신하고, 현재 플랜이 해당 결제로 반영된 상태면 `users.plan='free'`, `plan_expires_at=null`로 복구한다. 401 응답 시 `error: 'nicepay_auth'`, 그 외 실패는 `error: 'cancel_failed'`로 처리한다.
...
- **`/api/activity`**: GET — 연도별 활동 집계(`year` 쿼리). 홈 활동 그래프(ContributionGraph)에서 노트·커밋의 **생성·수정** 일별 횟수 조회(노트/커밋 모두 `created_at`, `updated_at` 반영). 삭제된 노트·커밋은 DB에서 제거되므로 해당 날짜 집계에서 제외됨. 클라이언트: 레이아웃에서 세션 확보 시 현재 연도 **프리페치**, `src/lib/activityCache.ts`로 연도별 캐시·진행 중 요청 공유 → 재진입 시 로딩 최소화.
...
- **`/api/pushmind/embed`**: POST — PushMind RAG용 embedding 동기화. Body `{}` | `{ noteId }` | `{ commitId }`. 노트·커밋 청크에 **태그, 카테고리, 상태, 연관 노트 제목, reference_urls, 첨부 파일명, created_at** 등 포함(하이브리드 확장). OpenAI Embeddings API 호출 후 `embeddings` 테이블 저장. 패널 열 때마다 전체 동기화(`{}`) 호출. 인증: 쿠키 또는 Bearer.
- **`/api/pushmind/chat`**: POST — PushMind 챗 질의. Body `{ message }`. **플랜별 동작**: **Free** — 구조적 쿼리만(최근 커밋·노트 개수 등); semantic 질문 시 "Pro 플랜 이상에서 이용 가능" 안내. **Pro/Team** — 하이브리드(구조적 쿼리 + RAG). **의도 분류**(규칙 + 필요 시 LLM 폴백): semantic/structural/hybrid. 구조적 쿼리: 최근/첫 커밋(N개), 커밋한 노트, 최근/처음/오래안수정 노트, 커밋 없는 노트, 연관 노트, 개수, 카테고리/태그/상태 필터, 특정 노트의 최신 커밋·연관 노트 등(`src/lib/pushmindStructured.ts`). 답변 + 출처 반환. 일일 한도 50회, `user_llm_usage` 누적. 인증: 쿠키 또는 Bearer. [PUSHMIND-RAG.md](./PUSHMIND-RAG.md) 참고.
...
| `NICE_PAY_SECRET_KEY` | ✅ | 나이스페이 REST 승인/취소 API 시크릿 키 (Basic 인증) | 서버 전용(Vercel 환경변수에 넣으면 브라우저에 노출되지 않음) |
| `NICE_PAY_API_BASE` | ✅ (실결제 시) | 나이스페이 REST API 도메인 | 미설정 시 sandbox 사용. **실결제(운영) 시 `https://api.nicepay.co.kr` 반드시 등록** (미등록 시 401/U116) |
