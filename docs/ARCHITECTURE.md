...
- **`/api/pushmind/chat`**: POST — PushMind 챗 질의. Body `{ message }`. **의도 분류**(규칙 + 필요 시 LLM 폴백): **semantic** → RAG만, **structural** → DB 구조적 쿼리만(`queryStructured`), **hybrid** → 쿼리 + RAG 병합. 구조적 쿼리: 최근/첫 커밋(N개), 커밋한 노트, 최근/처음/오래안수정 노트, 커밋 없는 노트, 연관 노트, 개수, 카테고리/태그/상태 필터, 특정 노트의 최신 커밋·연관 노트 등(`src/lib/pushmindStructured.ts`). 답변 + 출처 반환(구조적일 때는 structural sources만). 일일 한도 50회, `user_llm_usage` 누적. 인증: 쿠키 또는 Bearer. [PUSHMIND-RAG.md](./PUSHMIND-RAG.md) 참고.
