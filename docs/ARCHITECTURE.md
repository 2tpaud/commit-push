...
- **`/api/payment/return`**: 결제 완료/취소 후 콜백. GET은 취소로 간주. POST는 `authResultCode='0000'`일 때만 승인 API 호출 후 `payments`·`users.plan`·`plan_expires_at`·`notifications` 반영. **만료일**: 첫 구독·만료 후 = 결제일 기준 +1개월/12개월; 동일 플랜·만료일 미래 = 기존 만료일 기준 +1개월/12개월(return·webhook 동일). **승인 방식**: `NICE_PAY_MERCHANT_KEY`가 있으면 레거시(pay_process.jsp, AuthToken+SignData(가맹점키)만 사용·Basic 헤더 없음), 없으면 **v1 API**(`/v1/payments/{tid}`) + **Basic 인증**(clientId:secretKey). **키 타입**: v1 사용 시 클라이언트 키 **Server 승인**, 시크릿 키 **Basic 인증** 권장. 레거시 사용 시 승인 요청에 시크릿 키를 쓰지 않으나, 가맹점이 나이스페이에서 레거시/가맹점키 방식으로 설정된 경우 시크릿 키가 **Token 인증** 타입으로 발급된 경우가 있음(가맹점 설정 따름). PG 리다이렉트 시 세션 쿠키가 없으므로 **세션 없이** `SUPABASE_SERVICE_ROLE_KEY`로 `order_id` 기준 결제 조회·갱신. 응답은 PC/모바일 동일하게 **200 + HTML**(클라이언트 리다이렉트로 `/plan` 이동).
- **`/api/payment/cancel`**: 결제 승인 후 취소(환불) 처리. 인증: 쿠키 세션 우선, 없으면 `Authorization: Bearer`. 요청 사용자 본인 결제 건(`payments.id`)만 처리하며, `status='paid'`이고 `paid_at` 기준 24시간 이내일 때만 나이스페이 **레거시** 취소 API(`cancel_process.jsp`) 호출 후 `payments.status='cancelled'`로 갱신. 취소 API 서명은 **SignData = sha256(MID + CancelAmt + EdiDate + MerchantKey)** 이므로 `NICE_PAY_MERCHANT_KEY`를 설정해야 함(미설정 시 secretKey로 시도하나, 실결제에서 "가맹점키 조회 오류" A301 나오면 가맹점키 발급 후 설정). **실결제 시** 취소 URL은 `dc1-api.nicepay.co.kr`. 현재 플랜이 해당 결제로 반영된 상태면 `users.plan='free'`, `plan_expires_at=null`로 복구.
