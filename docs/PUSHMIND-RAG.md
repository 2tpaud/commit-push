...
- **답변 본문**: LLM에게 "해당 내용은 노트 N, 커밋 N에서 참고했어요" 문구를 넣지 말도록 지시(출처는 별도 블록으로만 표시). 만약 LLM이 여전히 이런 출처 문장을 본문에 섞어 넣더라도, 서버에서 후처리로 해당 문장들을 제거한 뒤 UI에 표시한다.

### 9.3 사용량 표시

- **프로필 드롭다운**(SharedAppLayout)과 **플랜 페이지**의 현재 사용량 카드에 PushMind 일일 사용량(`request_count`/50) 게이지가 노트·커밋과 함께 표시된다. `user_llm_usage` 테이블에서 당일 `request_count`를 조회한다.
