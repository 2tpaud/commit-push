...
- **returnUrl 동작**: 나이스페이는 인증 결과를 **POST**로만 전송하나, 결제창 닫기 시 모바일 등에서 **GET**으로만 오는 경우가 있음. `GET /api/payment/return`은 무조건 취소로 간주하고 `/plan?error=cancelled`로 리다이렉트. POST는 `authResultCode`가 `'0000'`이고 메시지에 취소/실패 문구가 없을 때만 승인 API 호출. ([나이스페이 로그/개발 가이드](https://start.nicepay.co.kr/manual/admin/developers/log/info.do) 참고)
...
4. 승인 실패 시: **payments**만 `status='failed'`로 UPDATE.
