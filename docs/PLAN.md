...
- **PushMind**: Free — 구조적 질문만(최근 커밋·노트 개수 등). Pro/Team — 하이브리드(구조적 쿼리 + 노트·커밋 내용 검색). Plan·pricing 페이지 플랜 카드에 기능 문구 반영(Free: "PushMind / 구조 기반 질문에 최적화된 AI", Pro/Team: "PushMind Hybrid / 맥락까지 이해하는 확장형 AI").
...
- **헤더 (상단 우측)** (로그인 후): **커밋푸시 아이콘**(MessageCircleMore), **새 노트 생성 아이콘**(FilePlus), **알림(벨) 아이콘**(Bell) → 클릭 시 각각 CommitPushDialog / NewNoteDialog / 알림 드롭다운. 그 우측에 **프로필 드롭다운**(아바타, 노트·커밋·PushMind 일일 사용량 게이지, Billing 링크, 로그아웃). 드롭다운 열 때마다 프로필(사용량) 재조회.
  - 프로필 드롭다운 사용량 상단에는 현재 플랜 배지를 표시하며, 유료 플랜(Pro/Team)인 경우 배지 옆에 `plan_expires_at` 만료일과 `D-N일`을 함께 표시.
- Plan 페이지에서 표시하는 내용:
  - 상단 **2열 레이아웃**: 좌측 **현재 사용량** 카드(플랜, 만료일, 노트/커밋/PushMind 일일 사용량 게이지), 우측 **청구 내역** 카드(`payments`에서 `status in ('paid','cancelled')` 조회, 승인일·금액·플랜·상태, 동작 컬럼 없음. 결제취소는 상태 셀에만 노출).
  - **월/연 구독 탭** + Free / Pro / Team 플랜 카드. **카드 선택**: 현재 플랜이 Pro/Team이면 **Free 카드는 선택 불가**(클릭 무시, opacity·tabIndex로 비활성). 그 외 카드는 클릭 시 해당 플랜 선택(링 강조).
  - **플랜 카드 버튼**: **Free 카드**는 하단 버튼 없음(현재 플랜일 때만 상단 "현재 플랜" 배지). **Pro/Team 카드**: **월 구독 탭**이고 현재 해당 플랜이면 **구독 취소**; **연 구독 탭**이거나 현재 플랜이 아니면 **결제하기**(탭에 따라 월/연 결제). 만료일 지난 뒤 재결제도 결제하기로 표시. 결제하기 클릭 시 `billingCycle`(탭 값)을 API에 전달해 연 구독 반영.
  - **Plan 페이지 로드 시**: `GET /api/plan/check-expiry` 호출 후 프로필 조회해 만료 시 `plan='free'` 반영.
  - **청구 내역 결제취소**: `status='paid'`이면서 `paid_at` 기준 24시간 이내 건만 **상태** 셀에 **결제취소** 버튼 노출.
...
| `NICE_PAY_SECRET_KEY` | REST 승인/취소 API 호출용 시크릿 키 (Basic 인증). **절대 클라이언트/저장소에 올리지 말 것.** | 서버 전용 |
...
| 변수명 | 용도 | 기본값 |
|--------|------|--------|
| `NICE_PAY_API_BASE` | REST 승인/취소 API 도메인 | `https://sandbox-api.nicepay.co.kr` (테스트) |
| `NEXT_PUBLIC_NICE_PAY_SDK_URL` | 결제창 JS 스크립트 URL | `https://sandbox-pay.nicepay.co.kr/v1/js/` (테스트) |

**운영 전환 시**: 위 두 기본값을 각각 `https://api.nicepay.co.kr`, `https://pay.nicepay.co.kr/v1/js/`로 변경하고, 클라이언트 키(**Server 승인**)·시크릿 키(v1 사용 시 **Basic 인증**, 레거시 사용 시 가맹점 설정에 따름)를 운영계 키로 교체해야 합니다.
...
- **승인 경로**: 나이스페이 **REST 승인 API** `POST {NICE_PAY_API_BASE}/v1/payments/{tid}` 한 가지 방식만 사용하며, 인증은 항상 **Basic** `Authorization: Basic base64(clientId:secretKey)`이다. (Access Token/Bearer/OAuth 흐름은 사용하지 않는다.)
- **승인 호출 조건**:
  - `authResultCode='0000'`
  - 메시지에 취소/실패 문구가 없을 것
...
- **성공 처리**:
  ...
  - `users.plan_expires_at` 설정: 동일 플랜이고 기존 만료일이 미래면 **기존 만료일 +1개월(월)/+12개월(연)**, 그 외(첫 구독·만료 후)는 **결제일 +1개월(월)/+12개월(연)**
  - `payments.status='paid'`, `tid`, `paid_at` 업데이트
  - `notifications`에 `type='payment_approved'` 1건 삽입
- **실패 처리**:
  - `payments.status='failed'` 업데이트
  - 플랜(`users`)은 변경하지 않음
- **상점명 표시**: 결제창·카드 청구서에 보이는 **상점명(가맹점명)** 은 API로 전달하지 않음. 나이스페이 가맹점 관리자(가맹점정보 > 기본정보)에서 설정하며, 미설정 시 PG사명(나이스페이 등)으로 표시될 수 있음. **상품명**은 `goodsName`으로 전달하며, 기본명(예: CommitPush Pro 1년) 뒤에 **이용 기간**(YYYY.MM.DD~YYYY.MM.DD)을 붙임. 기간은 결제일/만료일 기준(첫 구독·만료 후 = 결제일 기준, 동일 플랜·만료일 미래 = 기존 만료일 기준)으로 create 시점에 계산.
...
- **PG 연동**: 나이스페이 **REST 취소 API** `POST {NICE_PAY_API_BASE}/v1/payments/{tid}/cancel` 호출
  - 헤더: `Content-Type: application/json`, `Authorization: Basic base64(clientId:secretKey)` (Access Token/Bearer/OAuth 미사용).
  - 바디: `{ "orderId": payment.order_id, "amount": String(payment.amount), "reason": reason }`.
  - 성공 조건: 응답(또는 `result` 객체)의 `resultCode='0000'`. 401 응답 시 `error: 'nicepay_auth'`, 그 외 실패는 `error: 'cancel_failed'`로 처리.
- **성공 처리**:
...
- **결제 주기**: 플랜 페이지에서 **월 구독** / **연 구독 (20% 할인)** 탭 선택 가능. 연 구독은 월 구독료×12×0.8 (Pro 48,000원/년, Team 67,200원/년). **만료일**: 첫 구독 또는 만료 후 재구독 시 **결제일 기준** +1개월(월)/+12개월(연). 이미 같은 플랜 이용 중이고 만료일이 미래인 경우(연 구독 전환 등) **현재 만료일 기준** +1개월/+12개월. (return·webhook 둘 다 동일 로직.)
...
- 이 시점에서는 **users** 변경 없음. 결제창에 넘길 `orderId`, `amount`, `goodsName`(기본명+이용기간 예: CommitPush Pro 1년 (2025.02.16~2026.02.15)), `buyerName`(users.full_name), `buyerEmail`(로그인 사용자 이메일) 반환. Plan 페이지에서 `requestPay` 시 구매자 정보·상품명 전달.
...
| 테이블 | 동작 | 변경 내용 |
|--------|------|-----------|
| **users** | UPDATE | `plan` = 결제한 플랜(pro/team), `plan_expires_at` = 만료일(첫 구독·만료 후 = 결제일 기준 +1개월/12개월; 동일 플랜 만료일 미래 시 = 기존 만료일 +1개월/12개월), `updated_at` = now |
...
- **한도 상수**: `src/lib/planLimits.ts` — `PLAN_LIMITS`, `getLimitsForPlan()`, `PUSHMIND_DAILY_LIMIT` (Plan 페이지·헤더 게이지·PushMind 일일 사용량 공용)
- **헤더**: `src/components/SharedAppLayout.tsx` — 커밋푸시/새 노트 아이콘, **알림(벨) 아이콘**(결제 완료/취소/만료 임박 알림 표시), 프로필 드롭다운(노트·커밋·PushMind 일일 사용량 게이지, 유료 플랜 만료일·D-N, Billing, 로그아웃), NewNoteDialog·CommitPushDialog 렌더 (드롭다운 오픈 시 프로필 재조회)
