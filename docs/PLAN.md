...
| `NICE_PAY_SECRET_KEY` | 승인 API 호출용 시크릿 키. **v1 사용 시** Basic 인증 타입 권장(Token 시 U103 가능). **레거시(가맹점키) 사용 시** 나이스페이에서 Token 인증 타입으로 발급된 경우 많음(승인 요청에는 미사용). **절대 클라이언트/저장소에 올리지 말 것.** | 서버 전용 |
...
- **승인 경로**:
  - **`NICE_PAY_MERCHANT_KEY` 없음** (권장): **v1 API** `POST {NICE_PAY_API_BASE}/v1/payments/{tid}`, **Basic 인증**(Base64(clientId:secretKey)), body `{ amount }`. 이때 시크릿 키는 **Basic 인증** 타입으로 발급.
  - **`NICE_PAY_MERCHANT_KEY` 있음**: 레거시 **pay_process.jsp**로 form-urlencoded 승인(AuthToken, SignData(가맹점키)만 사용, Basic 헤더 없음). 가맹점키 방식 가맹점은 나이스페이에서 시크릿 키가 **Token 인증** 타입으로 발급된 경우가 있음(승인 요청에는 시크릿 키 미사용, 가맹점 설정 따름).
- **승인 호출 조건**:
  - `authResultCode='0000'`
  - 메시지에 취소/실패 문구가 없을 것
