...
| `NICE_PAY_MID` | 취소 API 서명·요청용 MID | 서버 전용 |
...
| `NICE_PAY_CANCEL_API_URL` | 취소 API URL | `{NICE_PAY_API_BASE}/webapi/cancel_process.jsp` |
...
- **PG 연동**: 나이스페이 취소 API(`application/x-www-form-urlencoded`) 호출
  - 기본 URL: `{NICE_PAY_API_BASE}/webapi/cancel_process.jsp`
...
- **처리 내용**: 수신 payload의 `signature` 검증(`hex(sha256(tid+amount+ediDate+SecretKey))`) 후 이벤트를 구분해 처리.
  - **승인 이벤트**(`resultCode=0000`, `status=paid`): `orderId`로 `payments` 조회 후 미승인이면 승인 API 호출 → `payments`·`users` 갱신 → `notifications`에 `payment_approved` 삽입(결제당 1건, `payment_id` unique).
  - **취소 이벤트**(API/관리자 취소): `orderId` 또는 `tid`로 `payments` 조회 후 `payments.status='cancelled'`, `users.plan='free'`, `plan_expires_at=null` 반영 및 `notifications`에 `payment_cancelled` 삽입.
  - 응답은 **Content-Type: text/html**, body **"OK"** 필수.
- **알림**: 결제 승인 시 `payment_approved`, 결제 취소 시 `payment_cancelled`를 `notifications`에 삽입. 헤더 **알림(벨) 아이콘**에서 조회·표시하며, 클릭 시 읽음 처리 및 결제 관련 알림이면 `/plan`으로 이동.
- **만료 임박 알림**: `/api/plan/check-expiry` 호출 시 유료 플랜의 만료일까지 남은 일수가 `3일` 또는 `1일`이면 각각 `plan_expiry_3days`, `plan_expiry_1day` 알림을 `notifications`에 삽입(동일 타입·동일 일자 중복 삽입 방지). 해당 알림 클릭 시 `/plan`으로 이동.
