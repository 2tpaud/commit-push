...
- **실패 처리**:
  ...
  - 플랜(`users`)은 변경하지 않음
- **상점명 표시**: 결제창·카드 청구서에 보이는 **상점명(가맹점명)** 은 API로 전달하지 않음. 나이스페이 가맹점 관리자(가맹점정보 > 기본정보)에서 설정하며, 미설정 시 PG사명(나이스페이 등)으로 표시될 수 있음. **상품명**은 `goodsName`(예: CommitPush Pro 1년)으로 전달 중.
...
- 이 시점에서는 **users** 변경 없음. 결제창에 넘길 `orderId`, `amount`, `goodsName`, `buyerName`(users.full_name), `buyerEmail`(로그인 사용자 이메일) 반환. Plan 페이지에서 `requestPay` 시 구매자 정보 전달.
