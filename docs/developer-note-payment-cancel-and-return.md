...
**결제 시 상점명이 "나이스페이" / "나이스정보통신(대리점)"으로 표시되는 경우**
- **구분**: **상품명(goodsName)** 은 코드에서 전달함(예: "CommitPush Pro 1년"). **상점명(가맹점명)** 은 카드 청구서·결제 완료 화면 등에 표시되는 "어디서 결제했는지" 이름으로, 나이스페이 **가맹점 관리자** 에서 등록하는 값임.
- **원인(상점명 미설정 시)**: 가맹점 관리자에 상점명을 설정하지 않으면 PG사명(나이스페이/나이스정보통신 등)으로 표시되는 경우가 많음. 결제창/승인 API에 상점명을 넘기는 파라미터는 없음.
- **원인(상점명 설정했는데도 "나이스정보통신(대리점)"으로 나올 때)**: **Basic 인증 / Token 인증 방식은 상점명 표시와 무관**함. API 인증 타입을 바꿔도 카드 청구·승인 화면의 상점명이 바뀌지 않음. "나이스정보통신(대리점)" 표시는 **대리점 가입(에이전시) 구조** 또는 **카드사·나이스페이 매입/표시 정책** 때문일 가능성이 큼. 대리점을 통해 가입한 경우 카드사 전산에서 1차로 대리점·PG사명이 노출되고, 실제 가맹점명은 별도 노출 정책에 따를 수 있음.
- **조치**: (1) 가맹점 관리자에서 **상점명**·**카드 청구서 표시명** 설정. (2) 설정 후에도 "나이스정보통신(대리점)"만 보이면 나이스페이(또는 가입 대리점)에 **"카드 청구서/승인 화면에 실 가맹점명(상호) 표시"** 가능 여부 문의(1661-0808 등). 직접가입·대리점 가입에 따라 표시 정책이 다를 수 있음.
- **참고**: 코드에서는 `goodsName`으로 상품명, `buyerName`(users.full_name)·`buyerEmail`(auth user.email)으로 구매자 정보를 결제창(`requestPay`)에 전달함. 상점명·표시명 변경은 PG 가맹점 관리자 및 PG/카드사 정책으로만 가능함.
