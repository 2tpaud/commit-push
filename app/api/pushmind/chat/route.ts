...
function postProcessAnswer(raw: string): string {
  let answer = raw
  // LLM이 실수로 답변 안에 "해당 내용은 노트 N에서 참고했어요" 같은 출처 문장을 넣는 경우 제거
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const rawAnswer = choice?.message?.content?.trim() ?? '답변을 생성하지 못했어요.'
